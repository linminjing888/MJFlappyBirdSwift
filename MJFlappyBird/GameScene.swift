...
class GameScene: SKScene ,SKPhysicsContactDelegate{
    ...
    /// 结点类 包含SKSpriteNode、SKLabelNode、SKVideoNode等
    /// 储存陆地、天空和管道
    var moving:SKNode!
    /// 储存所有上下管道
    var pipes:SKNode!
    ...
    override func didMove(to view: SKView) {
        ...
        let moveGroupSprite = SKAction.moveBy(x: -groundTexture.size().width * 2.0, y: 0, duration: TimeInterval(0.02 * groundTexture.size().width * 2.0))
        let resetGroupSprite = SKAction.moveBy(x: groundTexture.size().width * 2.0, y: 0, duration: 0.0)
        //永远移动 组动作
        ...
        let moveSkySpriteForever = SKAction.repeatForever(SKAction.sequence([moveSkySprite,resetSkySprite]))
        ...
        for i in 0..<2 + Int(self.frame.size.width / (skyTexture.size().width * 2)) {
            ...
            sprite.run(moveSkySpriteForever)
            moving.addChild(sprite)
        }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            //碰撞翻转
            bird.run(SKAction.rotate(byAngle: CGFloat(M_PI) * CGFloat(bird.position.y) * 0.01, duration: 1))
            // 背景闪光
            ...
            self.run(SKAction.sequence([SKAction.repeat(bgFlashAndNormal, count: 4), SKAction.run({self.canRestart = true})
             ]), withKey: "falsh")
